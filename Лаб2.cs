    ...
    class Program
     {
        ...
        static void Main()
        {
            ...
            //чтобы числа с точкой преобразовывались корректно в double
...
CultureInfo.GetCultureInfo("en");
...

        {
            ...
            */
            ...
            string[][] data = File.ReadAllLines(path).Select(x =>
x.Split('|')).ToArray();

            Model model = 0;
            double speed = 0;
            Model[] values =
Enum.GetValues(typeof(Model)).Cast<Model>().ToArray();
            return data
...
1 && choice > 5)
            ...
            public class Airplane
        {
            ...
            //в метод передается скорость, с которой сравнить скорость текущего самолета
            public bool Compare(double speed)
            {
                return Speed > speed;
            }
         
            //в метод передается экземпляр класса «Самолет», с которым сравнить текущий
